...
### Typography
Large, light-weight display serif (Cormorant Garamond) for headings — long ascenders, high contrast strokes, optically refined. Body in DM Sans at regular weight for clean readability. Eyebrow labels in all-caps DM Sans with wide tracking (`0.2em`). There is no monospace usage. Font weight is used sparingly; the brand leans light.
